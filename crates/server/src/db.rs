...
fn migrate_v6(conn: &rusqlite::Connection) -> Result<()> {
    ...
    // Fresh databases already have this column from SCHEMA; only add it for existing ones.
    let col_exists: bool = conn.query_row(
        "SELECT COUNT(*) FROM pragma_table_info('enforcement_settings') WHERE name='manual_locked'",
        [],
        |r| r.get::<_, i32>(0),
    ).unwrap_or(0) > 0;
    if !col_exists {
    ...
    tracing::info!("DB migration v6 applied (persistent manual lock)");
    Ok(())
}
...
mod tests {
    ...
    fn test_pool() -> DbPool {
        let pool = test_pool_before_v6();
        migrate_v6(&pool.get().unwrap()).unwrap();
        pool
    }
